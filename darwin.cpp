...
void reversesort(process** , int );
void sjf(process* , int );
int main()
{
	int n,clock;
	cout<<"Implementing Longest Job First\n\n";
	...
	cout<<"\n\nImplementing Shortest Jop First\n\n";
	sjf(s,n);
}
...
void reversesort(process *readyqueue[10],int n)
{
	for(int i=0;i<n-1;i++)
	{
		for(int j=0;j<n-i-1;j++)
...
void sjf(process *s,int n1)
{
	int n=n1;
	process *readyqueue1[n];
	...
	while(true)
	{
		...
		if(size>0 && executing == NULL)
		{
			reversesort(readyqueue1,size);
			readyqueue1[size-1]->exe=true;
			executing = readyqueue1[size-1];
			cout<<readyqueue1[size-1]->id<<" executed"<<endl;
			size--;
		}
		for(int j=0;j<size;j++)
		{
